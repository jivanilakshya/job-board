...
const EmployerDashboard = () => {
    ...
    const fetchData = async () => {
      try {
        const [jobsResponse, applicationsResponse] = await Promise.all([
          axios.get('http://localhost:5000/api/jobs/employer'),
  ...
  const handleJobStatusChange = async (jobId, status) => {
    try {
      await axios.patch(`http://localhost:5000/api/jobs/${jobId}`, { status });
                  ...
                  <div key={application._id} className="bg-white border rounded-lg p-4">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      <div className="flex-1">
                        ...
                        <p className="text-gray-600 mb-2">{application.name}</p>
                        <p className="text-gray-500 mb-2">{application.email}</p>
                        <div className="flex flex-wrap gap-2">
                          <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                            {application.status}
                          </span>
